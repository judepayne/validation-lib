...
class RuleLoader:
    ...
    def _load_single_rule(self, rule_id: str, entity_type: str = None) -> Any:
        ...
        if self.config_loader and self.rule_fetcher:
            # New mode: import directly from sys.path (logic_dir was added to sys.path)
            if not entity_type:
                # Try to infer entity type from rule_id context or search
                entity_type = self._infer_entity_type(rule_id)

            # Construct module path: rules.loan.rule_001_v1
            ...
            try:
                ...
                for alt_entity in ["loan", "facility", "deal"]:
                    if alt_entity != entity_type:
                        try:
                            module_name = f"rules.{alt_entity}.{rule_id}"
                            module = importlib.import_module(module_name)
                            entity_type = alt_entity
                            break
